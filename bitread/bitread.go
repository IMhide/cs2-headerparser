...
//
// This might be a contribution for the parser
//

func (r *BitReader)  NewParseString() string{
  length := r.ReadSignedInt(8)
  return r.ReadCString(length)
}


//
// Added by me for Investigation purpose 
//

func CustomLargeBitReader(underlying io.Reader, bufSize int32) *BitReader {
	b := make([]byte, bufSize)
	return newBitReader(underlying, &b)
}
